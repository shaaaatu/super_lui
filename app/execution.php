...
function executeRequest($request_type)
{
	...
	foreach ($service as $serviceName => $serviceLink)
	{
		...
		{
			$t->log("The service array for $serviceName service doesn't exist! Check config file");
            return;
        }
		if(!in_array($request_type, ${$serviceName}))
		{
             $t->log("Not found in $serviceName service");
             continue;
        }
        $t->log(
            "Request matched for $serviceName service>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
		);
        ...
        $t->log(
            "Data received from the `$serviceName` service in controller>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
    }
	return $output;
}
?>
